...
sort fix (same pressure with branch None and 'C' must not crash), plus the
v1.4.5 audit regressions.  The provenance / sha1 schema is a frozen contract
and is asserted field by field.
"""
import numpy as np
...
def test_full_group_single_stitch_and_a_junk_file(tmp_path):
    """A complete dark/background/sample trio with no .seq suffix gives one
    finite point, and an unparsable neighbour is skipped, not fatal."""
...
def test_partial_channels_are_raw_and_tagged(tmp_path):
    """Background-only, sample+dark, and a group with no pressure field:
    each loads, carries its tag, and has no absorbance."""
    _spec(tmp_path / "vis_D_glass_0_bg", 900.0)
    _spec(tmp_path / "vis_D_quartz_1p0_s", 600.0)
    _spec(tmp_path / "vis_D_quartz_1p0", 100.0)
    _spec(tmp_path / "vis_D_ice_bg", 800.0)          # no pressure -> 0 GPa
    results, skipped = _run(tmp_path)
    assert skipped == []              # a clean folder skips nothing
    by = {}
    for r in results:
        by.setdefault(r["sample"], []).append(r)

    assert len(by["glass"]) == 1
    assert "[B only]" in by["glass"][0]["label"]
    assert not np.isfinite(by["glass"][0]["absorbance"]).any()

    assert len(by["quartz"]) == 1
    assert "[S+D]" in by["quartz"][0]["label"]
    assert not np.isfinite(by["quartz"][0]["absorbance"]).any()

    assert len(by["ice"]) == 1 and by["ice"][0]["pressure_val"] == 0.0
...
def test_mixed_branch_does_not_crash(tmp_path):
    ...
    results, _skipped = _run(tmp_path)          # must not raise
    assert len([r for r in results if r["sample"] == "x"]) >= 2
...
def test_read_spectrum_tolerates_bom_and_bad_bytes(tmp_path):
    ...
    assert abs(cts[-1] - 3000.0) < 1e-9                # bad byte skipped
...
def test_channels_aligned_by_wavelength_not_index(tmp_path):
    ...
    res = engine.process_group(("Y1", "smp", "0", None),
                               {"meas": meas, "pressure_val": 0.0})
    assert len(res) == 1
    a = np.asarray(res[0]["absorbance"], float)
    fin = a[np.isfinite(a)]
    assert fin.size > 50
    assert np.nanmax(np.abs(fin)) < 1e-6, \
        "misaligned: max |A| = %g" % np.nanmax(np.abs(fin))
