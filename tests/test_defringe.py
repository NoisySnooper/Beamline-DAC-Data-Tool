...
The vendored core's own behaviour is covered in depth by
tests/test_fringe_core.py and tests/test_fringe_parity.py.
"""
import numpy as np
import defringe
...
def test_curve_contract_and_parameter_passthrough():
    """defringe_curve returns the same length, all finite, leaves a
    fringe-free curve alone, and honours every GUI-exposed knob."""
    wl = _wl()
    y = _clean_signal(wl)
    ...
    tuned = defringe.defringe_curve(wl, y, width_frac=0.10,
                                    nt_min_nm=20_000, nt_max_nm=80_000,
                                    pvalue_max=1e-3)
    assert tuned.shape == y.shape and np.isfinite(tuned).all()
